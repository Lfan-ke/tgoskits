...
unsafe extern "C" fn __boot_start() -> ! {
    ...
    1:
        ...
        # Record the saved boot regs ($s0..$s3 = a0..a3) into the plat statics for
        # DTB-based RAM detection, then enter call_main(cpuid, a1).
        ...
        move        $a1, $s1            # param2: boot a1 (conventional fdt/cmdline ptr) for call_main
        la.abs      $t0, {entry}
        li.d        $ra, 0
        jirl        $zero, $t0, 0",
