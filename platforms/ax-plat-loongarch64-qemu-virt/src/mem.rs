...
fn try_fdt_at(phys: usize, arr: &mut [RawRange; MAX_RAM_RANGES]) -> usize {
    if phys < 0x1000 || phys >= LOW_MEMORY_SIZE || !phys.is_multiple_of(4) {
        return 0;
    }
    ...
    let Some(vptr) = core::ptr::NonNull::new(vaddr as *mut u8) else {
        return 0;
    };
    let Ok(fdt) = Fdt::from_ptr(vptr) else {
        return 0;
    };
    let mut n = 0;
    for mem in fdt.memory() {
        for region in mem.regions() {
            ...
            arr[n] = (region.address as usize, region.size);
            n += 1;
        }
    }
    n
