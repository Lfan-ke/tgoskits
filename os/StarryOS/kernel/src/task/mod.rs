...
impl ProcessData {
    ...
    /// Mark the process stopped by `signo` and queue a `Stopped` report for
    /// the parent's `waitpid(WUNTRACED)`.
    ...
    pub fn peek_job_status_if(&self, want_stopped: bool, want_continued: bool) -> Option<JobStatus> {
    ...
    pub fn take_job_status_if(&self, want_stopped: bool, want_continued: bool) -> Option<JobStatus> {
        let mut jc = self.job_control.lock();
        match jc.status {
