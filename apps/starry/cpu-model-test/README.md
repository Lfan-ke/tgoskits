...
Without the submodule, set `MODEL_ASSET_SRC` / `PC_ASSET_SRC` to a `render-assets` tree. A missing model
or an unresolved LFS pointer fails the prebuild (exit 5), and a cell that cannot open an asset fails the run.
...
Every staged asset is required; no leg skips.
