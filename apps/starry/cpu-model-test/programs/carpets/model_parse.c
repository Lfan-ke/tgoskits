 ...
 *   - glTF/glb (suzanne.glb): parsed with vendored cgltf (reuse, not hand-rolled) - valid glTF 2.0,
 *     >=1 mesh/primitive.
 * Prebuild stages every asset these legs read, so a missing one fails the cell.
 */
...
int main(void) {
    ...
    if (have_cube) {
        ...
        fprintf(stderr, "  FAIL: cube derived assets missing under %s\n", model_dir());
        gate_check(&g, 0, "cube derived assets present");
    }
    ...
    if (have_suz) {
        ...
        fprintf(stderr, "  FAIL: suzanne.obj/.stl missing under %s\n", model_dir());
        gate_check(&g, 0, "suzanne assets present");
    }
    ...
    if (parse_stl(path, &bench) == 0 && bench.nt > 0) {
        ...
        fprintf(stderr, "  FAIL: benchy.stl missing under %s\n", model_dir());
        gate_check(&g, 0, "benchy asset present");
    }
    ...
    { FILE *f = fopen(path, "rb");
      if (!f) {
          fprintf(stderr, "  FAIL: suzanne.glb missing under %s\n", model_dir());
          gate_check(&g, 0, "glb asset present");
      } else {
          fclose(f);
