...
int main(void) {
    ...
    if (parse_ply(path, &sph) == 0 && sph.nv > 0) {
        ...
        fprintf(stderr, "  FAIL: sphere_pc.ply missing under %s\n", model_dir());
        gate_check(&g, 0, "sphere_pc asset present");
    }
    ...
    if (parse_ply(path, &bunny) == 0 && bunny.nv > 0) {
        ...
        fprintf(stderr, "  FAIL: bunny.ply missing under %s\n", model_dir());
        gate_check(&g, 0, "bunny asset present");
    }
