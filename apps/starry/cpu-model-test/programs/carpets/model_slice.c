 ...
 * Prebuild stages every mesh these legs read, so a missing one fails the cell.
 */
...
static void check_real(gate *g, const char *file, const layer_g *L, int nL, const char *tag) {
    ...
    if (parse_stl(path, &m) != 0 || m.nt == 0) {
        fprintf(stderr, "  FAIL: %s missing under %s\n", file, model_dir());
        gate_check(g, 0, "real slice asset present");
        return;
    }
...
int main(void) {
    ...
    if (have_cube) {
        ...
        fprintf(stderr, "  FAIL: cube.stl missing under %s\n", model_dir());
        gate_check(&g, 0, "cube asset present");
    }
