...
# render-assets tree, and a missing one fails the prebuild.
#
...
# Without git-lfs the submodule holds pointer files ("version https://git-lfs..."), not the real meshes.
is_lfs_pointer() {
    local p="$1" first
    [[ -f "$p" ]] || return 1
...
# Every real asset is required; a missing file or an unresolved LFS pointer fails the prebuild.
stage_real() {
    local src="$1" dst_dir="$2" name; name="$(basename "$src")"
    if [[ ! -f "$src" ]]; then
        echo "prebuild: required asset $name is missing (init the assets submodule)" >&2
        exit 5
    fi
    if is_lfs_pointer "$src"; then
        echo "prebuild: $name is an unresolved Git-LFS pointer (install git-lfs and run 'git lfs pull')" >&2
        exit 5
    fi
    cp -a "$src" "$dst_dir/" || exit 5
    echo "prebuild: staged $name"
}

# Stage the real models + point cloud under /opt/cpu-model-test/assets, then generate the derived
# closed-form assets (cube in five formats + synthetic sphere cloud) with tools/gen_goldens.py.
stage_assets() {
    local bin="$1" models pc
    mkdir -p "$bin/assets"
    models="$(find_models_src || true)"
    if [[ -z "${models:-}" || ! -d "$models" ]]; then
        echo "prebuild: real models not found (init the assets submodule or set MODEL_ASSET_SRC)" >&2
        exit 5
    fi
    ...
    pc="$(find_pc_src || true)"
    if [[ -z "${pc:-}" ]]; then
        echo "prebuild: point cloud not found (init the assets submodule or set PC_ASSET_SRC)" >&2
        exit 5
    fi
    stage_real "$pc/bunny.ply" "$bin/assets"
