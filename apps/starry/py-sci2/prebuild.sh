...
provision_conda() {
    ...
    local inst
    case "$arch" in
        x86_64)  inst=Miniforge3-26.3.2-3-Linux-x86_64.sh ;;
        aarch64) inst=Miniforge3-26.3.2-3-Linux-aarch64.sh ;;
    ...
    [[ -f "$cache" ]] || curl -fsSL -o "$cache" \
        "https://github.com/conda-forge/miniforge/releases/download/26.3.2-3/$inst"
    ...
    if [[ -x "$prefix_cache/bin/python" ]]; then
        ...
        [[ -f "$xcache" ]] || curl -fsSL -o "$xcache" \
            "https://github.com/conda-forge/miniforge/releases/download/26.3.2-3/$xinst"
...
stage_conda_glibc() {
    ...
    rm -rf "$t"
    # ctypes.util.find_library uses ldconfig; stage it + ld.so.conf so it works on StarryOS.
    deb_fetch "$binbdeb" "$binbdeb_sha" "$binbdeb" || true
    if [[ -f "$CONDA_DL/glibc/$binbdeb" ]]; then
        local t2; t2="$(mktemp -d)"
        ( cd "$t2" && ar x "$CONDA_DL/glibc/$binbdeb" && tar xf data.tar.* )
        real="$(find "$t2" -name ldconfig -type f 2>/dev/null | head -1)"
        [[ -n "$real" ]] && install -Dm0755 "$real" "$overlay_dir/sbin/ldconfig"
        rm -rf "$t2"
    fi
