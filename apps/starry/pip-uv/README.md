...
- **超时**: x86_64 / aarch64 / riscv64 为 600 秒；loongarch64 为 1200 秒（TCG 下 uv 较慢）

## 备注：为什么全部走本地包（离线）

当前 StarryOS 网络数据面（TLS 大段接收，#294 TLS-RX）尚未修复：三次握手（小包）可通，但较大的 TLS 数据段（ServerHello / Cert / wheel 下载）在接收路径上停滞，在线安装会超时甚至在网络任务上不可中断阻塞。因此本测试**不测在线 PyPI**，所有 pip / uv 操作均使用 `--no-index --find-links /opt/wheels`（pip）与本地二进制 + 本地 wheel（uv）的离线形式。在线安装能力已在主机基线与 httpsprobe 中验证，待网络数据面修复后再行覆盖。
