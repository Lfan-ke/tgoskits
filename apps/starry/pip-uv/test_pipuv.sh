...
# ---------- Stage 10a: pip install advanced options ----------
# --user install (pip installs into user site)
pip3 install --no-index --find-links /tmp/findlinks --user localpkg || fail "pip install --user (offline)"
python3 -c "import localmod" || fail "import localpkg after --user install"
# --upgrade / -U (force reinstall latest)
pip3 install --no-index --find-links /tmp/findlinks -U localpkg || fail "pip install -U (offline)"
# --no-cache-dir (disable cache)
pip3 install --no-cache-dir --no-index --find-links /tmp/findlinks --force-reinstall localpkg || fail "pip install --no-cache-dir (offline)"
# --compile / --no-compile (bytecode compilation toggle)
pip3 install --no-index --find-links /tmp/findlinks --force-reinstall --no-compile localpkg || fail "pip install --no-compile (offline)"
# --progress-bar off (non-interactive progress)
pip3 install --no-index --find-links /tmp/findlinks --force-reinstall --progress-bar off localpkg || fail "pip install --progress-bar off (offline)"
# -q / --quiet (quiet mode)
pip3 install -q --no-index --find-links /tmp/findlinks --force-reinstall localpkg || fail "pip install -q (offline)"
# --ignore-installed (ignore existing packages)
pip3 install --no-index --find-links /tmp/findlinks --ignore-installed localpkg || fail "pip install --ignore-installed (offline)"
next "pip-install-advanced-opts"

# ---------- Stage 10b: pip with requirements file ----------
echo "localpkg" > /tmp/req.txt
pip3 install --no-index --find-links /tmp/findlinks -r /tmp/req.txt --force-reinstall || fail "pip install -r (offline)"
next "pip-requirements-file"
...
# ---------- Stage 11a: pip config debug ----------
pip3 config debug >/dev/null 2>&1 || fail "pip config debug"
pip3 config list 2>&1 >/dev/null || fail "pip config list"
next "pip-config"

# ---------- Stage 11b: pip cache management ----------
pip3 cache dir >/dev/null || fail "pip cache dir"
pip3 cache info >/dev/null || fail "pip cache info"
pip3 cache list 2>&1 >/dev/null || true  # may be empty
next "pip-cache"

# ---------- Stage 11c: pip hash ----------
echo "hashme" > /tmp/hash_in
pip3 hash /tmp/hash_in 2>&1 | grep -q "sha256" || fail "pip hash sha256"
pip3 hash -a sha384 /tmp/hash_in 2>&1 | grep -q "sha384" || fail "pip hash sha384"
next "pip-hash"

# ---------- Stage 11d: pip debug ----------
pip3 debug --verbose 2>&1 | grep -q "pip version" || fail "pip debug"
next "pip-debug"

# ---------- Stage 11e: pip inspect ----------
pip3 inspect localpkg 2>&1 >/dev/null || true  # inspect installed package
LOCAL_WHEEL="$(ls /tmp/findlinks/localpkg-*.whl 2>/dev/null | head -1)"
test -n "$LOCAL_WHEEL" && pip3 inspect "$LOCAL_WHEEL" 2>&1 >/dev/null || true
next "pip-inspect"
...
# ---------- Stage 15a: uv pip compile ----------
echo "localpkg" > /tmp/req.in
uv pip compile --python /root/uvv/bin/python --no-index --find-links /tmp/findlinks /tmp/req.in >/tmp/compiled.txt 2>&1 || fail "uv pip compile"
grep -q "localpkg" /tmp/compiled.txt || fail "uv pip compile output missing localpkg"
next "uv-pip-compile"

# ---------- Stage 15b: uv pip uninstall ----------
uv pip uninstall --python /root/uvv/bin/python -y localpkg || fail "uv pip uninstall localpkg"
/root/uvv/bin/python -c "import localmod" 2>/dev/null && fail "localpkg still importable after uv uninstall" || true
uv pip install --python /root/uvv/bin/python --no-index --find-links /tmp/findlinks localpkg || fail "uv pip reinstall localpkg"
next "uv-pip-uninstall"

# ---------- Stage 15c: uv pip check ----------
uv pip check --python /root/uvv/bin/python 2>&1 >/dev/null || true  # check consistency
next "uv-pip-check"
...
# ---------- Stage 17a: uv python list / find ----------
uv python list 2>&1 >/dev/null || fail "uv python list"
uv python find 2>&1 >/dev/null || fail "uv python find"
uv python dir 2>&1 >/dev/null || fail "uv python dir"
next "uv-python-list-find"

# ---------- Stage 17b: uv cache prune / clean ----------
uv cache prune 2>&1 >/dev/null || true  # non-fatal
uv cache dir >/dev/null || fail "uv cache dir (recheck)"
next "uv-cache-prune"

# ---------- Stage 17c: uv self version ----------
uv self version 2>&1 >/dev/null || true  # reports current version
next "uv-self-version"

# ---------- Stage 17d: uv tool dir / list ----------
uv tool dir 2>&1 >/dev/null || fail "uv tool dir"
uv tool list 2>&1 >/dev/null || true  # may be empty
next "uv-tool-dir-list"

# ---------- Stage 17e: uv pip tree ----------
uv pip tree --python /root/uvv/bin/python 2>&1 >/dev/null || true  # show dep tree
next "uv-pip-tree"

# ---------- Stage 17f: uv python pin ----------
uv python pin 2>&1 >/dev/null || true  # pin Python version
next "uv-python-pin"

# ---------- Stage 17g: uv lock / uv export (project operations) ----------
mkdir -p /tmp/uvproj
cat > /tmp/uvproj/pyproject.toml << 'PYEOF'
[project]
name = "testproj"
version = "0.1.0"
requires-python = ">=3.9"
PYEOF
( cd /tmp/uvproj && uv lock 2>&1 >/dev/null ) || true  # generate lock file
( cd /tmp/uvproj && uv export 2>&1 >/dev/null ) || true  # export requirements
next "uv-lock-export"
