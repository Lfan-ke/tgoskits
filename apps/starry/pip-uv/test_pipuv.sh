...
OUT="$(uv run --no-project python3 -c 'print(1)' 2>/dev/null | tail -1)"
...
print("uv-script-ok")
PYEOF
uv run --no-project --script /tmp/s.py 2>&1 | grep -q "uv-script-ok" || fail "uv run --no-project --script (PEP 723)"
next "uv-run-script"
