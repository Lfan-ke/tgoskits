...
# Unpacking the base image is the longest step of a run: debugfs walks the whole
# 6 GiB rootfs and emits an ownership warning per inode. The result only depends
# on the image, so a stamp of what it was unpacked from lets a rerun skip it -
# which is most reruns, since the image changes far less often than the app does.
base_stamp() {
    stat --format=%s:%Y "$base_rootfs" 2>/dev/null
}

extract_base_rootfs() {
    local stamp_file="$staging_root/.base-rootfs-stamp"
    local stamp
    stamp=$(base_stamp)
    if [ -n "$stamp" ] && [ -f "$stamp_file" ] && [ "$(cat "$stamp_file")" = "$stamp" ]        && [ -x "$staging_root/sbin/apk" ]; then
        echo "[browser prebuild] staging root already unpacked from this image, skipping rdump"
        return
    fi
    rm -rf "$staging_root"
...
REPO
    ...
    local zlib_url="https://dl-cdn.alpinelinux.org/alpine/v3.23/main/${arch}/zlib-1.3.2-r0.apk"
    ...
    echo "[browser prebuild] installing Firefox-ESR + Weston via qemu-user apk..."
    ...
    LD_LIBRARY_PATH="$staging_root/lib:$staging_root/usr/lib" \
        "$qemu_runner" -L "$staging_root" \
            ...
            add weston weston-backend-drm weston-shell-desktop \
                firefox-esr mesa-gl mesa-egl mesa-dri-gallium mesa-gbm \
                gtk+3.0 nss nspr icu-libs dbus-libs libevent \
                ca-certificates fontconfig font-dejavu font-wqy-zenhei \
                gsettings-desktop-schemas shared-mime-info \
                adwaita-icon-theme hicolor-icon-theme \
                libinput libxkbcommon pixman xkeyboard-config \
                fbgrab

    # apk --no-scripts skipped every post-install trigger, so the caches GTK/GdkPixbuf
    # need at runtime are missing. Build them with the HOST's native tools - these
    # caches are architecture-independent data - not qemu-user (which cannot resolve
    # the glib libraries these tools link). Critically: an EMPTY gdk-pixbuf
    # loaders.cache makes gdk-pixbuf register NO loaders, not even the built-in PNG
    # one, so GTK cannot decode its own icons ("Unrecognized image file format") and
    # aborts. Delete the cache instead - with no cache file gdk-pixbuf falls back to
    # its built-in loaders (PNG included), which is what firefox's chrome icons need.
    rm -f "$staging_root/usr/lib/gdk-pixbuf-2.0/2.10.0/loaders.cache"
    if command -v update-mime-database >/dev/null 2>&1; then
        echo "[browser prebuild] building shared-mime-info mime.cache (host, ~minutes)..."
        update-mime-database "$staging_root/usr/share/mime" >/dev/null 2>&1 \
            && echo "[browser prebuild] mime.cache built" || echo "[browser prebuild] mime.cache skipped"
    fi
    if command -v gtk-update-icon-cache >/dev/null 2>&1; then
        for theme in Adwaita hicolor; do
            [[ -d "$staging_root/usr/share/icons/$theme" ]] && \
                gtk-update-icon-cache -f -t "$staging_root/usr/share/icons/$theme" >/dev/null 2>&1 \
                && echo "[browser prebuild] $theme icon-theme.cache built" || true
        done
    fi
}
...
ensure_host_packages
resize_rootfs "$base_rootfs" 6144
extract_base_rootfs
install_packages
populate_overlay
