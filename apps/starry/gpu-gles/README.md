# gpu-gles (G-side, virtio-gpu vGPU)

Placeholder reserving the GPU-side counterpart of the CPU-side software carpet
`cpu-gles` (#1610). It exercises the same OpenGL ES 3.1 API surface and the same full
language-binding cartesian (C / C++ / Rust / Python / JS / TS / Kotlin) as the
CPU side, but against the qemu virtio-gpu virtual GPU device (virtio-gpu GLES (virgl over the 3D render node)) under
